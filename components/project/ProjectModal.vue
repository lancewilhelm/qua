...
<template>
    <BaseModal v-if="showModal && project" @close="closeModal" @submit="$emit('submit')">
        ...
        <div class="grid justify-center items-center grid-cols-1">
            ...
            <input v-model="project.name" type="text" autocomplete="off" placeholder="project name" name="name">
            <div class="font-mono text-main text-left">descritpion</div>
            <textarea v-model="project.description" placeholder="project description" name="description" rows="7"
                maxlength="250" />
            <div class="font-mono text-main text-left">tags</div>
            <div class="flex flex-row items-start mb-1">
                <input v-model="tagInput" type="text" autocomplete="off" class="w-36 mr-2" placeholder="enter tag"
                    name="tags" @keydown.enter.stop="() => {
                        if (tagInput !== '' && project) {
                            if (!project.tags) {
                                project.tags = []
                            }
                ...
                <div class="flex flex-row flex-wrap self-center">
                    <div v-for="tag in project.tags" :key="tag"
                        ...
                        {{ tag }}
                        <div class="cursor-pointer hover:text-error active:text-error-extra" @click="deleteTag(tag)">
            ...
            <div class="grid gap-4 grid-cols-2">
                ...
                <button type="button" class="modal-btn" @click="$emit('submit')" @keydown.enter.prevent="null">
